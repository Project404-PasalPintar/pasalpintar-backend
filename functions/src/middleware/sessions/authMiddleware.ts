...
export const verifyAccessToken = (
  ...
  verifyToken(token, accessTokenSecret, "access", (err, decodedAccess) => {
    if (decodedAccess) {
      ...
      if (decodedAccess.role !== "user" && decodedAccess.role !== "lawyer") {
        return res.status(403).json({
          status: "fail",
          message:
            "Insufficient permissions. Only students or lawyers can access this route.",
        });
      }

      req.user = {
        ...decodedAccess,
        userID, // Pastikan userID diisi dari id yang terdecode
      };
      return next();
    }
    ...
    verifyToken(
      ...
      (errRefresh, decodedRefresh) => {
        ...
        if (decodedRefresh) {
          ...
          // Check if the role is valid (either 'user' or 'lawyer')
          if (
            decodedRefresh.role !== "user" &&
            decodedRefresh.role !== "lawyer"
          ) {
            return res.status(403).json({
              status: "fail",
              message:
          ...
          req.user = {
            ...decodedRefresh,
            userID,
          }; // Refresh token is valid and role is valid
          return next();
        }
