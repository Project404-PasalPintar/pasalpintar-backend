...
export const signUpUser = async (newTutorData: user, res: Response) => {
  try {
    ...
    // Use userRecord.uid as the document ID in the 'users' collection
    ...
    await bucket
      ...
      .save("This is the root folder for user " + userRecord.uid);

    // Store the data in the 'userData' subcollection's 'profileToValidate' document
...
export const signInUser = async (
  ...
  try {
    ...
    // Gunakan userRecord.uid untuk mendapatkan data user dari Firestore
    const tutorDocRef = admin
      .firestore()
      .collection("testing")
      .doc("data")
    ...
    await tutorDocRef.update({
      fcmToken: fcmToken || null, // Simpan fcmToken di dokumen utama users
    });
    ...
    jwt.verify(token, JWT_REFRESH_TOKEN_SECRET, async (err, user) => {
      ...
      try {
        ...
        if (!profileDoc.exists) {
          return reject(new Error("user data is missing."));
        }
        ...
        const newAccessToken = jwt.sign(
          {
            ...
            role: (user as jwt.JwtPayload).role,
          },
...
export const signOutUser = async (token: string, res: Response) => {
  try {
    ...
    // Retrieve the user document to verify the refresh token
    const tutorDocRef = admin
      .firestore()
      .collection("testing")
      .doc("data")
